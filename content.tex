...
\subsection{Somme fermée de parties d'un espace vectoriel normé}
\begin{exo}
	Soit $E$ un espace vetoriel normé. Soient $F$ une partie fermée de $E$, $K$ une partie compacte de $E$. Montrer que $F + K$ est une partie fermée de $E$.
\end{exo}

\begin{sol}
	Soit $(u_n)$ une suite d'éléments de $F + K$, supposons qu'elle converge vers $\ell \in E$. Montrons que $\ell \in F + K$. Pour $n\in\N$, $u_n\in F + K$, il existe alors $x_n\in F$, $y_n\in K$ tels que $u_n=x_n + y_n$. Puis $(y_n)$ est une suite de $K$ compact, il existe alors $\varphi$ une application strictement croissante de $\N$ dans $\N$ telle que $(y_{\varphi(n)})$ converge vers $\ell'\in K$. Comme pour tout $n\in \N$, $x_{\varphi(n)}=u_{\varphi(n)}-y_{\varphi(n)}$ la suite $(x_{\varphi(n)})$ converge vers $\ell'\in F$ (l'appartenance de la limite à $F$ est garentie par ceci que $F$ est une partie fermée de $E$), et on a $\ell' = \ell - \ell'$ soit $\ell=\ell''+\ell'\in F + K$. Ce qui achève de montrer que $F + K$ est fermée, car ceci vaut pour toute suite convergente de $F + K$.
\end{sol}
...
\begin{exo}
	Soit $a\in\Rpe$ et $f\in\mathcal C^\infty([0, a[,\R)$ telle $f^{(n)}\geq 0$ pour tout entier naturel $n$.
	\begin{enumerate}
		...
		\item En déduire que pour tout $x\in[0,a[$, $f(x)=\sum_{n=0}^{+\infty}\frac{f^{(n)}}{n!}x^n$
		\item En utilisation la question 2), démontrer que la fonction tangente est développable en série entière à l'origine et préciser l'intervalle de validité de ce développement.
	\end{enumerate}
\end{exo}
...
\begin{exo}
	Soit $M\in\mathcal M_n(\K)$. On appelle mineur principal d'ordre $k\in\lbrace1,\dots,n\rbrace$ le déterminant d'un $M_I=(m_{i,j})_{(i,j)\in I^2}$ avec $I\subset\lbrace 1,\dots,n\rbrace$ tel que $\Card(I)=k$. Donner une expression des coefficients de $\Xi_M$ en fonction des mineurs principaux.
\end{exo}
...
\newpage
\section[Intégrales généralisées, Intégrales à paramètres]{Intégrales généralisées, Intégrales à paramètres}
\subsection{Une Intégrale de Frullani}
\begin{exo}
	On pose pour $x\in\R$, $f(x)=\in_0^{+\infty}\frac{\arctan(xt)-\arctan(t)}t\dd t$
	\begin{enumerate}
		...
		\item Retrouver le résultat de la question 3) sans utiliser le théor!me de dérivation des intégrales à paramètre\\
	\end{enumerate}
\end{exo}
...
\subsection{Une intégrale à paramètre}
\begin{exo}
	$F(x)=\int_0^{+\infty}\frac{\sin(tx)}{e^t-1}\dd t$
	\begin{enumerate}
		\item Montrer que $F$ est définie et continue sur $\R$ puis $\mathcal C^1$ sur $\R$
		\item Montrer que $\forall t\in \R$, $F(t)=\sum_{n=1}^{+\infty}\frac1{n^2+t^2}$. Retrouver $F'$.
		\item Montrer que $F$ est $\C^{+\infty}$ sur $\R$.
	\end{enumerate}
\end{exo}
...
\newpage
\section[Espaces euclidiens]{Espaces euclidiens}
...
\begin{exo}
	...
	\begin{enumerate}
		...
		\item Montrer que si $A\in\mathcal A_n(\R)$ (ensemble des matrices antisymétriques), alors $\text{Sp}(A)\subset i\R$;
		\item Montrer que $\varphi : M \mapsto (I_n-M)(I_n+M)^{-1}$ définit une involution de $\mathcal E$;
		\item Montrer que $\varphi$ induit une bijection $\tilde\varphi$ de $\mathcal A_n(\R)$ sur $\mathcal{SO}_n(\R)\cap \mathcal E$.
	\end{enumerate}
\end{exo}
